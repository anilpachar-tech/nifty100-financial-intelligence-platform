...
selected_group = st.selectbox("Select Peer Group", groups["peer_group_name"])

peer_data = get_latest_peer_data(selected_group)

# ------------------------------------
# Company Dropdown
# ------------------------------------

selected_company = st.selectbox("Select Company", peer_data["company_name"])

company = peer_data[peer_data["company_name"] == selected_company]
...
with col1:
    ...
    st.metric("ROCE", round(latest["return_on_capital_employed_pct"], 2))
...
with col2:
    ...
    st.metric("Revenue CAGR", round(latest["revenue_cagr_5yr"], 2))

    st.metric("PAT CAGR", round(latest["pat_cagr_5yr"], 2))
...
categories = ["ROE", "ROCE", "NPM", "D/E", "FCF", "Revenue CAGR", "PAT CAGR", "Quality"]
...
def normalize(company, peer):
    ...
    for c, p in zip(company, peer):
        ...
        if mx == 0:
            ...
            values.append(((c / mx) * 100, (p / mx) * 100))
...
company_values, peer_values = normalize(company_values, peer_values)

fig = go.Figure()

fig.add_trace(
    go.Scatterpolar(
        r=company_values, theta=categories, fill="toself", name=latest["company_name"]
    )
)

fig.add_trace(
    go.Scatterpolar(r=peer_values, theta=categories, fill="toself", name="Peer Average")
)

fig.update_layout(
    polar=dict(radialaxis=dict(visible=True)), showlegend=True, height=600
)
...
styled_df = peer_data.style.apply(highlight_benchmark, axis=1)
