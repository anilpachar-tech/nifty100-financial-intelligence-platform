...
df = df.dropna(
    subset=["broad_sector", "sales", "return_on_equity_pct", "market_cap_crore"]
)
...
with m1:
    st.metric("💰 Median Revenue", f"₹{sector_df['sales'].median():,.0f} Cr")

with m2:
    st.metric("📈 Median ROE", f"{sector_df['return_on_equity_pct'].median():.2f}%")

with m3:
    st.metric(
        "🏦 Median Market Cap", f"₹{sector_df['market_cap_crore'].median():,.0f} Cr"
    )
...
fig.update_traces(
    marker=dict(opacity=0.85, line=dict(width=1)),
...
median_df = (
    sector_df[["sales", "return_on_equity_pct", "market_cap_crore"]]
    .median()
    .reset_index()
)
...
display_df = display_df.sort_values(by="market_cap_crore", ascending=False)
...
display_df["Revenue (₹ Cr)"] = display_df["Revenue (₹ Cr)"].map(lambda x: f"₹{x:,.0f}")

display_df["ROE (%)"] = display_df["ROE (%)"].map(lambda x: f"{x:.2f}%")
...
highest_roe = display_df.sort_values("ROE (%)", ascending=False).iloc[0]["Company"]

st.success(f"🏆 Largest Company by Market Cap: **{highest_revenue}**")

st.info(f"📈 Best ROE Company: **{highest_roe}**")
