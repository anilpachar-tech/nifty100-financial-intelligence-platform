...
companies = sorted(profile["company_name"].dropna().unique())

selected_company = st.selectbox("Search Company", companies)

company = profile[profile["company_name"] == selected_company]

latest = company.sort_values("year").iloc[-1]
...
with col5:
    st.metric("Revenue CAGR (5Y)", f"{latest['revenue_cagr_5yr']:.2f}%")

with col6:
    st.metric("Free Cash Flow", round(latest["free_cash_flow_cr"], 2))

trend = get_company_trend(latest["id"])
...
trend_ratio = get_roe_roce_trend(latest["id"])
...
pros_cons = get_pros_cons(latest["id"])
